...
'
    ...
    // neochat reward
    public function NeochatReward(){
        $reward=1000;
        $max=3;
        DB::table('users')->where('id',Auth::guard('users')->user()->id)->update([
            'giftcard_balance' => DB::raw('giftcard_balance + '.$reward.'')
        ]);
        if(DB::table('transactions')->where('type','Neo Chat')->where('status','success')->where('user_id',Auth::guard('users')->user()->id)->whereDate('date',Carbon::today())->count() >= $max){
            return response()->json([
                'message' => 'error',
                'status' => 'error'
            ]);
...
',
    ...
    // login claim
    public function LoginClaim(){
        DB::table('users')->where('id',Auth::guard('users')->user()->id)->update([
            'dailyclaim_balance' => DB::raw('dailyclaim_balance + 5'),
            'last_claim' => Carbon::now()
        ]);

        return response()->json([
            'balance' => number_format(Auth::guard('users')->user()->dailyclaim_balance + 5),
