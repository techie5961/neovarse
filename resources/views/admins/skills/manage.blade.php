...
@section('main')
                ...
                {{-- loop card --}}
                    <div style="border:1px solid var(--rgb-01)" class="w-full bg-light br-10 p-10">
                        <div class="row w-full g-10">
                           <img style="border:1px solid var(--rgb-005)" src="{{ asset('skills/'.$data->photo.'') }}" alt="" class="h-70 perfect-square br-10 no-shrink">
                            <div class="column g-5">
                        ...
                        <div>Community : <a href="{{ $data->community }}" class="c-primary">Visit Community</a></div>
                        <div class="w-full">
                        ...
                        <div class="row align-center space-between w-full">
                            <button onclick="window.location.href='{{ url('admins/skills/edit?id='.$data->id.'') }}'" class="btn-blue">
                             ...
                             <button onclick="MyFunc.Delete('{{ $data->name }}','{{ $data->id }}')" class="btn-red">
                                Delete
    ...
    <template class="popup-template">
        <div class="w-full g-10 p-10 text-center column">
            ...
            <span class="font-1">Are you sure you want to delete this Skill( <span class="name">Programming</span> )? this action is irrevisible</span>
            <button onclick="window.location.href='{{ url('admins/skills/delete?id=') }}' + this.dataset.id" class="btn-red w-full">Yes! Delete this skill</button>
        </div>
    </template>
@endsection
@section('js')
